...
#if defined(NDEBUG)
# error "Deancoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x29c87ace63a87b8be0154d270dc4b860b55b6f5df5c5ced7172bae79271cb0b8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Deancoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Deancoin Signed Message:\n";
...
{
    // Deancoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Deancoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Deancoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Deancoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Deancoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Deancoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x079f9e2ed83d37db4966eb97599523c254ae9915099fd302326dc01ff1555e2b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "It's Fun learning about coins";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04781aa9381c2cb19820e43770619049b8125b949d2bbd1bb7093eaf2eb4e2207fbec39005362a03c54b1c1b321543e6571e254633e1c9a56e05372e14ed83e28f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1527457373;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086449393;

        if (fTestNet)
        {
            block.nTime    = 1527457344;
            block.nNonce   = 385531167;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x05eb8d5f661b217f71fef4fba8682e4d98bb36098c699a7260a6657997f7d8ca"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Deancoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DeancoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DeancoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DeancoinMiner(CWallet *pwallet)
{
    printf("DeancoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("deancoin-miner");
    ...
    try { loop {
        ...
        printf("Running DeancoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DeancoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DeancoinMiner, pwallet));
}
